...
#ifdef BNB_ENABLE_INSTRUMENTATION
// Simple counters only - no timing
__device__ unsigned int d_nf4_lookup_count = 0;
__device__ unsigned int d_scaling_factor_count = 0;
__device__ unsigned int d_memory_access_count = 0;
__device__ unsigned int d_total_kernel_calls = 0;

// Host storage for counters
static unsigned int h_nf4_lookup_count = 0;
static unsigned int h_scaling_factor_count = 0;
static unsigned int h_memory_access_count = 0;
static unsigned int h_total_kernel_calls = 0;
#endif
...
__device__ __forceinline__ float dDequantizeNF4(unsigned char val) {
    #ifdef BNB_ENABLE_INSTRUMENTATION
        // Simple counter increment - no timing overhead
        atomicInc(&d_nf4_lookup_count, UINT_MAX);
    #endif
        
        // Original NF4 dequantization tree
        if ((val & 0b1000) == 8) {
            if ((val & 0b0100) == 4) {         // 1
                if ((val & 0b0010) == 2) {     // 11
                    if ((val & 0b0001) == 1) { // 111
...
#ifdef BNB_ENABLE_INSTRUMENTATION
    // Track kernel invocation (only once per kernel launch)
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        atomicInc(&d_total_kernel_calls, UINT_MAX);
    }
#endif
...
#ifdef BNB_ENABLE_INSTRUMENTATION
        // Count scaling factor access
        atomicInc(&d_scaling_factor_count, UINT_MAX);
#endif
...
#ifdef BNB_ENABLE_INSTRUMENTATION
        // Count memory access
        atomicInc(&d_memory_access_count, UINT_MAX);
#endif
...
#pragma unroll NUM_PER_TH
            ...
            break;
        case NF4:
            // NF4 dequantization - this calls dDequantizeNF4 which increments counter
#pragma unroll NUM_PER_TH
            for (int j = 0; j < NUM_PER_TH; j++) {
...
// Function to extract counter data from GPU to CPU  
void extract_bnb_counter_data() {
    cudaMemcpyFromSymbol(&h_nf4_lookup_count, d_nf4_lookup_count, sizeof(unsigned int));
    cudaMemcpyFromSymbol(&h_scaling_factor_count, d_scaling_factor_count, sizeof(unsigned int));
    cudaMemcpyFromSymbol(&h_memory_access_count, d_memory_access_count, sizeof(unsigned int));
    cudaMemcpyFromSymbol(&h_total_kernel_calls, d_total_kernel_calls, sizeof(unsigned int));
    cudaDeviceSynchronize();
}

void get_nf4_lookup_count(unsigned int* count) {
    *count = h_nf4_lookup_count;
}

void get_scaling_factor_count(unsigned int* count) {
    *count = h_scaling_factor_count;
}

void get_memory_access_count(unsigned int* count) {
    *count = h_memory_access_count;
}

void get_kernel_call_count(unsigned int* count) {
    *count = h_total_kernel_calls;
}

void reset_bnb_counters() {
    unsigned int zero = 0;
    cudaMemcpyToSymbol(d_nf4_lookup_count, &zero, sizeof(unsigned int));
    cudaMemcpyToSymbol(d_scaling_factor_count, &zero, sizeof(unsigned int));
    cudaMemcpyToSymbol(d_memory_access_count, &zero, sizeof(unsigned int));
    cudaMemcpyToSymbol(d_total_kernel_calls, &zero, sizeof(unsigned int));
    
    h_nf4_lookup_count = 0;
    h_scaling_factor_count = 0;
    h_memory_access_count = 0;
    h_total_kernel_calls = 0;
...
#endif // BNB_ENABLE_INSTRUMENTATION
